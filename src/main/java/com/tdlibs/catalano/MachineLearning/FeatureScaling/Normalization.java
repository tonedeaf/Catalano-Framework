...
package com.tdlibs.catalano.MachineLearning.FeatureScaling;
...
public class Normalization implements IFeatureScaling{
    ...
    public void ApplyInPlace(double[][] data){
        ...
        for (int i = 0; i < data[0].length; i++) {
            ...
            for (int j = 0; j < temp.length; j++) {
                data[j][i] = com.tdlibs.catalano.Math.Tools.Scale(_min, _max, min, max, temp[j]);
            }
        }
    }
    ...
    public double[] Compute(double[] feature){
        ...
        for (int i = 0; i < norm.length; i++) {
            double v = com.tdlibs.catalano.Math.Tools.Scale(range[0][i], range[1][i], min, max, feature[i]);
            v = v > 1 ? 1 : v;
            v = v < 0 ? 0 : v;
            norm[i] = v;
        }
    ...
    public double[] Compute(DecisionVariable[] attributes, double[] feature){
        ...
        for (int i = 0; i < norm.length; i++) {
            int idx = 0;
            if(attributes[i].type == DecisionVariable.Type.Continuous){
                double v = com.tdlibs.catalano.Math.Tools.Scale(range[0][idx], range[1][idx], min, max, feature[i]);
                idx++;
