...
class BBDTree {
    ...
    private double filter(Node node, double[][] centroids, int[] candidates, int k, double[][] sums, int[] counts, int[] membership) {
        ...
        double minDist = com.tdlibs.catalano.Math.Distances.Distance.SquaredEuclidean(node.center, centroids[candidates[0]]);
        int closest = candidates[0];
        for (int i = 1; i < k; i++) {
            double dist = com.tdlibs.catalano.Math.Distances.Distance.SquaredEuclidean(node.center, centroids[candidates[i]]);
            if (dist < minDist) {
                minDist = dist;
