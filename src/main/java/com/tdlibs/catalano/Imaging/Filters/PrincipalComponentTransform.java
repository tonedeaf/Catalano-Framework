...
import com.tdlibs.catalano.Math.Decompositions.SingularValueDecomposition;
import com.tdlibs.catalano.Math.Matrix;
import com.tdlibs.catalano.Statistics.Tools;
    ...
    public FastBitmap Extract(Component component){
        ...
        if(component == Component.Red){
            ...
            double max = Matrix.Max(Matrix.getColumn(image, 0));
             ...
             for (int i = 0; i < size; i++) {
                int c = (int)com.tdlibs.catalano.Math.Tools.Scale(min, max, 0, 255, image[i][0]);
                fb.setGray(i, c);
            }
            ...
            double max = Matrix.Max(Matrix.getColumn(image, 1));
             ...
             int size = width * height;
             for (int i = 0; i < size; i++) {
                int c = (int)com.tdlibs.catalano.Math.Tools.Scale(min, max, 0, 255, image[i][1]);
                fb.setGray(i, c);
            }
            ...
            double max = Matrix.Max(Matrix.getColumn(image, 2));
             ...
             int size = width * height;
             for (int i = 0; i < size; i++) {
                int c = (int)com.tdlibs.catalano.Math.Tools.Scale(min, max, 0, 255, image[i][2]);
                fb.setGray(i, c);
            }
            ...
            double maxBlue = Matrix.Max(Matrix.getColumn(image, 2));
             ...
             int size = width * height;
             for (int i = 0; i < size; i++) {
                int r = (int)com.tdlibs.catalano.Math.Tools.Scale(minRed, maxRed, 0, 255, image[i][0]);
                int g = (int)com.tdlibs.catalano.Math.Tools.Scale(minGreen, maxGreen, 0, 255, image[i][1]);
                int b = (int)com.tdlibs.catalano.Math.Tools.Scale(minBlue, maxBlue, 0, 255, image[i][2]);
