...
public class FastRadialSymmetryTransform implements IApplyInPlace{
    ...
    public void applyInPlace(FastBitmap fastBitmap) {
        ...
        if (fastBitmap.isGrayscale()){
            ...
            // Scale image
            double min = com.tdlibs.catalano.Math.Matrix.Min(symmetryMap);
            double max = com.tdlibs.catalano.Math.Matrix.Max(symmetryMap);
            
            for (int i = 0; i < height; i++) {
                for (int j = 0; j < width; j++) {
                    symmetryMap[i][j] = com.tdlibs.catalano.Math.Tools.Scale(min, max, 0, 255, symmetryMap[i][j]);
                }
            }
