...
public class ImageUtils {
    ...
    public static void Normalize(int[][] image, int min, int max){
        ...
        for (int i = 0; i < image.length; i++) {
            for (int j = 0; j < image[0].length; j++) {
                image[i][j] = (int)com.tdlibs.catalano.Math.Tools.Scale(mm[0], mm[1], min, max, image[i][j]);
            }
        }
    }
    ...
    public static void Normalize(double[][][] image, double min, double max){
        ...
        for (int i = 0; i < image.length; i++) {
            for (int j = 0; j < image[0].length; j++) {
                image[i][j][0] = (int)com.tdlibs.catalano.Math.Tools.Scale(minRed, maxRed, min, max, image[i][j][0]);
                image[i][j][1] = (int)com.tdlibs.catalano.Math.Tools.Scale(minGreen, maxGreen, min, max, image[i][j][1]);
                image[i][j][2] = (int)com.tdlibs.catalano.Math.Tools.Scale(minBlue, maxBlue, min, max, image[i][j][2]);
            }
        }
    }
}